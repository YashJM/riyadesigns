...
function FeaturedCard({
  ...
  return (
    <article className="rounded-[clamp(22px,2.1vw,30px)] bg-[rgba(243,135,171,0.2)] px-[clamp(16px,4vw,22px)] py-[clamp(18px,4vw,24px)] md:px-[clamp(14px,1.6vw,22px)] md:py-[clamp(14px,1.6vw,22px)]">
      <div
    ...
    <section className="figma-gradient relative w-full overflow-hidden">
      <div className="figma-shell relative isolate min-h-[min(100svh,760px)] pb-[var(--space-hero-pad-bottom)] pt-[var(--space-hero-pad-top)] md:min-h-[clamp(620px,64vw,820px)] md:pb-[clamp(4rem,10vw,9.125rem)] md:pt-[clamp(5.25rem,11vw,9.625rem)]">
        <Image
          ...
          height={108}
          aria-hidden
          className="pointer-events-none absolute left-[max(4px,env(safe-area-inset-left))] top-[clamp(48px,12vw,94px)] h-[clamp(48px,12vw,108px)] w-[clamp(52px,14vw,125px)] md:left-[clamp(8px,3vw,42px)]"
        />
        <Image
          ...
          height={264}
          aria-hidden
          className="pointer-events-none absolute right-[max(4px,env(safe-area-inset-right))] top-[clamp(200px,58vw,378px)] h-[clamp(100px,32vw,264px)] w-[clamp(102px,34vw,268px)] md:right-[clamp(10px,7vw,94px)] md:top-[clamp(270px,41vw,378px)]"
        />

        <p className="relative z-10 max-w-full text-[clamp(18px,4.6vw,32px)] leading-[1.15] font-semibold">
          👋 UI/UX Designer &amp; Product Strategist
        </p>

        <div className="relative z-10 mt-[clamp(14px,3vw,30px)] max-w-[min(100%,1094px)]">
        ...
        <p className="relative z-10 mt-[clamp(18px,4vw,36px)] max-w-[min(100%,58ch)] text-[clamp(16px,4.2vw,28px)] leading-[1.36] text-[#222222]">
        ...
        <Link
          href="#featured"
          className="absolute bottom-[max(12px,env(safe-area-inset-bottom))] left-1/2 z-10 inline-flex h-14 w-14 -translate-x-1/2 items-center justify-center transition-transform duration-300 ease-out hover:-translate-x-1/2 hover:-translate-y-1 focus-visible:outline-none sm:h-16 sm:w-16 md:bottom-[clamp(22px,3vw,36px)]"
...
export function FigmaHomeFrame() {
  return (
    <div className="bg-[#fffdfb]">
      ...
      >
        <div className="mb-[clamp(1.25rem,4vw,2rem)] border-b border-separator pb-[clamp(1rem,3vw,1.25rem)]">
        ...
        <div className="flex flex-col gap-[var(--space-stack-gap)]">
          {featured.flatMap((item, index) => {
            const block = [<FeaturedCard key={item.slug} {...item} />];
